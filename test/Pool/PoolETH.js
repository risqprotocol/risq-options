...
module.exports.test = () => contract("ETHPool", ([user1, user2, user3]) => {
  ...
  it("Should transfer ownership", async () => {
    const {TestETHPool} = await contracts
    await TestETHPool.transferOwnership(user3)
  })

  it("Should mint tokens for the first provider correctly", async () => {
    const {TestETHPool} = await contracts
    await TestETHPool.provide(0, {value: firstProvide, from: user1})
    assert(
      await TestETHPool.shareOf(user1).then((x) => x.eq(firstProvide)),
      "Wrong amount"
    )
  })
  ...
  it("Should mint tokens for the second provider correctly", async () => {
    const {TestETHPool} = await contracts
    await TestETHPool.provide(0, {value: secondProvide, from: user2})
    assert(
      await TestETHPool.shareOf(user2).then((x) => x.eq(secondProvide)),
      "Wrong amount"
    )
  })
  ...
  it("Should distribute the profits correctly", async () => {
    ...
    const [startShare1, startShare2] = await Promise.all([
      TestETHPool.shareOf(user1),
      TestETHPool.shareOf(user2),
    ])
  ...
  it("Should mint tokens for the third provider correctly", async () => {
    ...
    await TestETHPool.provide(0, {value, from: user3})
    assert.isAtLeast(
      await TestETHPool.shareOf(user3).then((x) => x.sub(value).toNumber()),
      -1,
      "The third provider has lost funds"
    )
    assert(
      await TestETHPool.shareOf(user1).then((x) => x.eq(startShare1)),
      "The first provider has an incorrect share"
    )
    assert(
      await TestETHPool.shareOf(user2).then((x) => x.eq(startShare2)),
  ...
  it("Should burn the first provider's tokens correctly", async () => {
    ...
    const [startShare1, startShare2, startShare3] = await Promise.all([
      TestETHPool.shareOf(user1),
      TestETHPool.shareOf(user2),
      TestETHPool.shareOf(user3),
    ])

    await timeTravel(14 * 24 * 3600 + 1)
    const gasPrice = await web3.eth.getGasPrice().then((x) => new BN(x))
    const transactionFee = await TestETHPool.withdraw(value, MAX_INTEGER)
