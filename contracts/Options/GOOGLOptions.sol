...
// File: contracts/Pool/RisqGOOGLPool.sol
...
/**
 * @author macemclain
 * @title Risq GOOGL Liquidity Pool
 * @notice Accumulates liquidity in GOOGL from LPs and distributes P&L in GOOGL
 */
contract RisqGOOGLPool is
    IERCLiquidityPool,
    Ownable,
    BEP20("Risq GOOGL LP Token", "writeGOOGL")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies GOOGL to the pool and receives writeGOOGL tokens
    ...
    /*
     * @nonce Provider burns writeGOOGL and receives GOOGL from the pool
     * @param amount Amount of GOOGL to receive
    ...
    /*
     * @nonce Returns provider's share in GOOGL
     * @param account Provider's address
     * @return Provider's share in GOOGL
     */
    ...
    /*
     * @nonce Returns the amount of GOOGL available for withdrawals
    ...
    /*
     * @nonce Returns the GOOGL total balance provided to the pool
...
// File: contracts/Options/RisqGOOGLOptions.sol
...
/**
 * @author macemclain
 * @title Risq GOOGL (Wrapped Mirror GOOGL Token) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqGOOGLOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    RisqGOOGLPool public pool;
    IPancakeRouter01 public pancakeRouter;
    address[] public ethToGooglSwapPath;
    IBEP20 public googl;

    /**
     * @param _priceProvider The address of ChainLink GOOGL/USD price feed contract
     * @param _pancake The address of Pancakeswap router contract
     * @param token The address of GOOGL BEP20 token contract
     */
    constructor(
        ...
        ethToGooglSwapPath = new address[](2);
        ethToGooglSwapPath[0] = pancakeRouter.WETH();
        ethToGooglSwapPath[1] = address(googl);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToGOOGL(totalETH, total);
    ...
    function approve() public {
        googl.safeApprove(address(pool), uint(-1));
        googl.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = pancakeRouter.getAmountsIn(total, ethToGooglSwapPath)[0];
    }
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of GOOGL
    ...
    /**
     * @notice Sends profits in GOOGL from the GOOGL pool to an option holder's address
    ...
    /**
     * @notice Swap BNB to GOOGL via Pancakeswap router
     * @param maxAmountIn The maximum amount of BNB that can be required before the transaction reverts.
     * @param amountOut The amount of GOOGL tokens to receive.
     */
    function swapToGOOGL(
        uint maxAmountIn,
        uint amountOut
    )
        internal
