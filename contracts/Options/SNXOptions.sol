...
import "../Pool/SNXPool.sol";


/**
 * @author macemclain
 * @title Risq SNX (Synthetix) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqSNXOptions is Ownable, IRisqOptions {
    ...
    address[] public ethToSnxSwapPath;
    IERC20 public snx;
     ...
     * @param token The address of SNX ERC20 token contract
     */
    constructor(
        ...
        ethToSnxSwapPath = new address[](2);
        ethToSnxSwapPath[0] = uniswapRouter.WETH();
        ethToSnxSwapPath[1] = address(snx);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToSNX(totalETH, total);
    ...
    function approve() public {
        snx.safeApprove(address(pool), uint(-1));
        snx.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = uniswapRouter.getAmountsIn(total, ethToSnxSwapPath)[0];
    }
    ...
    /**
     * @notice Sends profits in SNX from the SNX pool to an option holder's address
