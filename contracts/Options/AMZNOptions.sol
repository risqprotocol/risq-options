...
// File: contracts/Pool/RisqAMZNPool.sol
...
/**
 * @author macemclain
 * @title Risq AMZN Liquidity Pool
 * @notice Accumulates liquidity in AMZN from LPs and distributes P&L in AMZN
 */
contract RisqAMZNPool is
    IERCLiquidityPool,
    Ownable,
    BEP20("Risq AMZN LP Token", "writeAMZN")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies AMZN to the pool and receives writeAMZN tokens
    ...
    /*
     * @nonce Provider burns writeAMZN and receives AMZN from the pool
     * @param amount Amount of AMZN to receive
    ...
    /*
     * @nonce Returns provider's share in AMZN
     * @param account Provider's address
     * @return Provider's share in AMZN
     */
    ...
    /*
     * @nonce Returns the amount of AMZN available for withdrawals
    ...
    /*
     * @nonce Returns the AMZN total balance provided to the pool
...
// File: contracts/Options/RisqAMZNOptions.sol
...
/**
 * @author macemclain
 * @title Risq AMZN (Wrapped Mirror AMZN Token) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqAMZNOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    RisqAMZNPool public pool;
    IPancakeRouter01 public pancakeRouter;
    address[] public ethToAmznSwapPath;
    IBEP20 public amzn;

    /**
     * @param _priceProvider The address of ChainLink AMZN/USD price feed contract
     * @param _pancake The address of Pancakeswap router contract
     * @param token The address of AMZN BEP20 token contract
     */
    constructor(
        ...
        ethToAmznSwapPath = new address[](2);
        ethToAmznSwapPath[0] = pancakeRouter.WETH();
        ethToAmznSwapPath[1] = address(amzn);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToAMZN(totalETH, total);
    ...
    function approve() public {
        amzn.safeApprove(address(pool), uint(-1));
        amzn.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = pancakeRouter.getAmountsIn(total, ethToAmznSwapPath)[0];
    }
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of AMZN
    ...
    /**
     * @notice Sends profits in AMZN from the AMZN pool to an option holder's address
    ...
    /**
     * @notice Swap BNB to AMZN via Pancakeswap router
     * @param maxAmountIn The maximum amount of BNB that can be required before the transaction reverts.
     * @param amountOut The amount of AMZN tokens to receive.
     */
    function swapToAMZN(
        uint maxAmountIn,
        uint amountOut
