...
// File: contracts/Pool/RisqCAKEPool.sol
...
/**
 * @author macemclain
 * @title Risq CAKE Liquidity Pool
 * @notice Accumulates liquidity in CAKE from LPs and distributes P&L in CAKE
 */
contract RisqCAKEPool is
    IERCLiquidityPool,
    Ownable,
    BEP20("Risq CAKE LP Token", "writeCAKE")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies CAKE to the pool and receives writeCAKE tokens
    ...
    /*
     * @nonce Provider burns writeCAKE and receives CAKE from the pool
     * @param amount Amount of CAKE to receive
    ...
    /*
     * @nonce Returns provider's share in CAKE
     * @param account Provider's address
     * @return Provider's share in CAKE
     */
    ...
    /*
     * @nonce Returns the amount of CAKE available for withdrawals
    ...
    /*
     * @nonce Returns the CAKE total balance provided to the pool
...
// File: contracts/Options/RisqCAKEOptions.sol
...
/**
 * @author macemclain
 * @title Risq CAKE (PancakeSwap Token) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqCAKEOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    RisqCAKEPool public pool;
    IPancakeRouter01 public pancakeRouter;
    address[] public ethToCakeSwapPath;
    IBEP20 public cake;

    /**
     * @param _priceProvider The address of ChainLink CAKE/USD price feed contract
     * @param _pancake The address of Pancakeswap router contract
     * @param token The address of CAKE BEP20 token contract
     */
    constructor(
        ...
        ethToCakeSwapPath = new address[](2);
        ethToCakeSwapPath[0] = pancakeRouter.WETH();
        ethToCakeSwapPath[1] = address(cake);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToCAKE(totalETH, total);
    ...
    function approve() public {
        cake.safeApprove(address(pool), uint(-1));
        cake.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = pancakeRouter.getAmountsIn(total, ethToCakeSwapPath)[0];
    }
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of CAKE
    ...
    /**
     * @notice Sends profits in CAKE from the CAKE pool to an option holder's address
    ...
    /**
     * @notice Swap BNB to CAKE via Pancakeswap router
     * @param maxAmountIn The maximum amount of BNB that can be required before the transaction reverts.
     * @param amountOut The amount of CAKE tokens to receive.
     */
    function swapToCAKE(
        uint maxAmountIn,
        uint amountOut
