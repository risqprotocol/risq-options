...
import "../Pool/DEFIPool.sol";


/**
 * @author macemclain
 * @title Risq sDEFI (Synth sDEFI) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqDEFIOptions is Ownable, IRisqOptions {
    ...
    address[] public ethToSdefiSwapPath;
    IERC20 public sdefi;
     ...
     * @param token The address of sDEFI ERC20 token contract
     */
    constructor(
        ...
        ethToSdefiSwapPath = new address[](2);
        ethToSdefiSwapPath[0] = uniswapRouter.WETH();
        ethToSdefiSwapPath[1] = address(sdefi);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToSDEFI(totalETH, total);
    ...
    function approve() public {
        sdefi.safeApprove(address(pool), uint(-1));
        sdefi.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = uniswapRouter.getAmountsIn(total, ethToSdefiSwapPath)[0];
    }
    ...
    /**
     * @notice Sends profits in sDEFI from the sDEFI pool to an option holder's address
    ...
    /**
     * @notice Swap ETH to sDEFI via Uniswap router
     * @param maxAmountIn The maximum amount of ETH that can be required before the transaction reverts.
     * @param amountOut The amount of sDEFI tokens to receive.
     */
    function swapToSDEFI(
        uint maxAmountIn,
        uint amountOut
    )
        internal
