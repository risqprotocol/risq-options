...
// File: contracts/Pool/RisqYFIPool.sol
...
/**
 * @author macemclain
 * @title Risq YFI Liquidity Pool
 * @notice Accumulates liquidity in YFI from LPs and distributes P&L in YFI
 */
contract RisqYFIPool is
    IERCLiquidityPool,
    Ownable,
    BEP20("Risq YFI LP Token", "writeYFI")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies YFI to the pool and receives writeYFI tokens
    ...
    /*
     * @nonce Provider burns writeYFI and receives YFI from the pool
     * @param amount Amount of YFI to receive
    ...
    /*
     * @nonce Returns provider's share in YFI
     * @param account Provider's address
     * @return Provider's share in YFI
     */
    ...
    /*
     * @nonce Returns the amount of YFI available for withdrawals
    ...
    /*
     * @nonce Returns the YFI total balance provided to the pool
...
// File: contracts/Options/RisqYFIOptions.sol
...
/**
 * @author macemclain
 * @title Risq YFI (Binance-Peg yearn.finance) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqYFIOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    RisqYFIPool public pool;
    IPancakeRouter01 public pancakeRouter;
    address[] public ethToYfiSwapPath;
    IBEP20 public yfi;

    /**
     * @param _priceProvider The address of ChainLink YFI/USD price feed contract
     * @param _pancake The address of Pancakeswap router contract
     * @param token The address of YFI BEP20 token contract
     */
    constructor(
        ...
        ethToYfiSwapPath = new address[](2);
        ethToYfiSwapPath[0] = pancakeRouter.WETH();
        ethToYfiSwapPath[1] = address(yfi);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToYFI(totalETH, total);
    ...
    function approve() public {
        yfi.safeApprove(address(pool), uint(-1));
        yfi.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = pancakeRouter.getAmountsIn(total, ethToYfiSwapPath)[0];
    }
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of YFI
    ...
    /**
     * @notice Sends profits in YFI from the YFI pool to an option holder's address
    ...
    /**
     * @notice Swap BNB to YFI via Pancakeswap router
     * @param maxAmountIn The maximum amount of BNB that can be required before the transaction reverts.
     * @param amountOut The amount of YFI tokens to receive.
     */
    function swapToYFI(
        uint maxAmountIn,
        uint amountOut
