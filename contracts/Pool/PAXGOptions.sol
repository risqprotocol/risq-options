...
// File: contracts/Pool/RisqPAXGPool.sol
...
/**
 * @author macemclain
 * @title Risq PAXG Liquidity Pool
 * @notice Accumulates liquidity in PAXG from LPs and distributes P&L in PAXG
 */
contract RisqPAXGPool is
    IERCLiquidityPool,
    Ownable,
    BEP20("Risq PAXG LP Token", "writePAXG")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies PAXG to the pool and receives writePAXG tokens
    ...
    /*
     * @nonce Provider burns writePAXG and receives PAXG from the pool
     * @param amount Amount of PAXG to receive
    ...
    /*
     * @nonce Returns provider's share in PAXG
     * @param account Provider's address
     * @return Provider's share in PAXG
     */
    ...
    /*
     * @nonce Returns the amount of PAXG available for withdrawals
    ...
    /*
     * @nonce Returns the PAXG total balance provided to the pool
...
// File: contracts/Options/RisqPAXGOptions.sol
...
/**
 * @author macemclain
 * @title Risq PAXG (PAX Gold Token on BSC) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqPAXGOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    RisqPAXGPool public pool;
    IPancakeRouter01 public pancakeRouter;
    address[] public ethToPaxgSwapPath;
    IBEP20 public paxg;

    /**
     * @param _priceProvider The address of ChainLink PAXG/USD price feed contract
     * @param _pancake The address of Pancakeswap router contract
     * @param token The address of PAXG BEP20 token contract
     */
    constructor(
        ...
        ethToPaxgSwapPath = new address[](2);
        ethToPaxgSwapPath[0] = pancakeRouter.WETH();
        ethToPaxgSwapPath[1] = address(paxg);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToPAXG(totalETH, total);
    ...
    function approve() public {
        paxg.safeApprove(address(pool), uint(-1));
        paxg.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = pancakeRouter.getAmountsIn(total, ethToPaxgSwapPath)[0];
    }
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of PAXG
    ...
    /**
     * @notice Sends profits in PAXG from the PAXG pool to an option holder's address
    ...
    /**
     * @notice Swap BNB to PAXG via Pancakeswap router
     * @param maxAmountIn The maximum amount of BNB that can be required before the transaction reverts.
     * @param amountOut The amount of PAXG tokens to receive.
     */
    function swapToPAXG(
        uint maxAmountIn,
        uint amountOut
