...
// File: contracts/Pool/RisqETHPool.sol
...
contract RisqETHPool is
    IERCLiquidityPool,
    Ownable,
    BEP20("Risq ETH LP Token", "writeETH")
{
    using SafeMath for uint256;
...
// File: contracts/Options/RisqETHOptions.sol
...
/**
 * @author macemclain
 * @title Risq ETH (Binance-Peg Ethereum Token) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqETHOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    RisqETHPool public pool;
    IPancakeRouter01 public pancakeRouter;
    address[] public ethToEthSwapPath;
    IBEP20 public eth;
     ...
     * @param token The address of ETH BEP20 token contract
     */
    constructor(
        ...
        ethToEthSwapPath = new address[](2);
        ethToEthSwapPath[0] = pancakeRouter.WETH();
        ethToEthSwapPath[1] = address(eth);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToETH(totalETH, total);
    ...
    function approve() public {
        eth.safeApprove(address(pool), uint(-1));
        eth.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = pancakeRouter.getAmountsIn(total, ethToEthSwapPath)[0];
    }
    ...
    /**
     * @notice Swap BNB to ETH via Pancakeswap router
     * @param maxAmountIn The maximum amount of BNB that can be required before the transaction reverts.
     * @param amountOut The amount of ETH tokens to receive.
     */
    function swapToETH(
        uint maxAmountIn,
        uint amountOut
