...
// File: contracts/Pool/RisqLTCPool.sol
...
/**
 * @author macemclain
 * @title Risq LTC Liquidity Pool
 * @notice Accumulates liquidity in LTC from LPs and distributes P&L in LTC
 */
contract RisqLTCPool is
    IERCLiquidityPool,
    Ownable,
    BEP20("Risq LTC LP Token", "writeLTC")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies LTC to the pool and receives writeLTC tokens
    ...
    /*
     * @nonce Provider burns writeLTC and receives LTC from the pool
     * @param amount Amount of LTC to receive
    ...
    /*
     * @nonce Returns provider's share in LTC
     * @param account Provider's address
     * @return Provider's share in LTC
     */
    ...
    /*
     * @nonce Returns the amount of LTC available for withdrawals
    ...
    /*
     * @nonce Returns the LTC total balance provided to the pool
...
// File: contracts/Options/RisqLTCOptions.sol
...
/**
 * @author macemclain
 * @title Risq LTC (Binance-Peg Litecoin Token) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqLTCOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    RisqLTCPool public pool;
    IPancakeRouter01 public pancakeRouter;
    address[] public ethToLtcSwapPath;
    IBEP20 public ltc;

    /**
     * @param _priceProvider The address of ChainLink LTC/USD price feed contract
     * @param _pancake The address of Pancakeswap router contract
     * @param token The address of LTC BEP20 token contract
     */
    constructor(
        ...
        ethToLtcSwapPath = new address[](2);
        ethToLtcSwapPath[0] = pancakeRouter.WETH();
        ethToLtcSwapPath[1] = address(ltc);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToLTC(totalETH, total);
    ...
    function approve() public {
        ltc.safeApprove(address(pool), uint(-1));
        ltc.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = pancakeRouter.getAmountsIn(total, ethToLtcSwapPath)[0];
    }
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of LTC
    ...
    /**
     * @notice Sends profits in LTC from the LTC pool to an option holder's address
    ...
    /**
     * @notice Swap BNB to LTC via Pancakeswap router
     * @param maxAmountIn The maximum amount of BNB that can be required before the transaction reverts.
     * @param amountOut The amount of LTC tokens to receive.
     */
    function swapToLTC(
        uint maxAmountIn,
        uint amountOut
