...
// File: contracts/Staking/RisqStakingTSLA.sol
...
contract RisqStakingTSLA is RisqStaking, IRisqStakingBEP20 {
    using SafeBEP20 for IBEP20;
    using SafeMath for uint;

    IBEP20 public immutable TSLA;

    constructor(BEP20 _token, BEP20 tsla) public
        RisqStaking(_token, "RISQ TSLA Staking lot", "rTSLA") {
        TSLA = tsla;
    }
    ...
    function sendProfit(uint amount) external override {
        ...
        if (_totalSupply > 0) {
            ...
            TSLA.safeTransferFrom(msg.sender, address(this), amount);
            emit Profit(amount);
        } else {
            TSLA.safeTransferFrom(msg.sender, FALLBACK_RECIPIENT, amount);
        }
    }

    function _transferProfit(uint amount) internal override {
        TSLA.safeTransfer(msg.sender, amount);
    }
}