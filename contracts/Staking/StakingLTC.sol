...
// File: contracts/Staking/RisqStakingLTC.sol
...
contract RisqStakingLTC is RisqStaking, IRisqStakingBEP20 {
    using SafeBEP20 for IBEP20;
    using SafeMath for uint;

    IBEP20 public immutable LTC;

    constructor(BEP20 _token, BEP20 ltc) public
        RisqStaking(_token, "RISQ LTC Staking lot", "rlLTC") {
        LTC = ltc;
    }
    ...
    function sendProfit(uint amount) external override {
        ...
        if (_totalSupply > 0) {
            ...
            LTC.safeTransferFrom(msg.sender, address(this), amount);
            emit Profit(amount);
        } else {
            LTC.safeTransferFrom(msg.sender, FALLBACK_RECIPIENT, amount);
        }
    }

    function _transferProfit(uint amount) internal override {
        LTC.safeTransfer(msg.sender, amount);
    }
}