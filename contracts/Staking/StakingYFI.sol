...
// File: contracts/Staking/RisqStakingYFI.sol
...
contract RisqStakingYFI is RisqStaking, IRisqStakingBEP20 {
    using SafeBEP20 for IBEP20;
    using SafeMath for uint;

    IBEP20 public immutable YFI;

    constructor(BEP20 _token, BEP20 yfi) public
        RisqStaking(_token, "RISQ YFI Staking lot", "rYFI") {
        YFI = yfi;
    }
    ...
    function sendProfit(uint amount) external override {
        ...
        if (_totalSupply > 0) {
            ...
            YFI.safeTransferFrom(msg.sender, FALLBACK_RECIPIENT, amount);
        }
    }
