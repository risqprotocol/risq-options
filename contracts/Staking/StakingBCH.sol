...
// File: contracts/Staking/RisqStakingBCH.sol
...
contract RisqStakingBCH is RisqStaking, IRisqStakingBEP20 {
    using SafeBEP20 for IBEP20;
    using SafeMath for uint;

    IBEP20 public immutable BCH;

    constructor(BEP20 _token, BEP20 bch) public
        RisqStaking(_token, "RISQ BCH Staking lot", "rBCH") {
        BCH = bch;
    }
    ...
    function sendProfit(uint amount) external override {
        ...
        if (_totalSupply > 0) {
            ...
            BCH.safeTransferFrom(msg.sender, address(this), amount);
            emit Profit(amount);
        } else {
            BCH.safeTransferFrom(msg.sender, FALLBACK_RECIPIENT, amount);
        }
    }

    function _transferProfit(uint amount) internal override {
        BCH.safeTransfer(msg.sender, amount);
    }
}