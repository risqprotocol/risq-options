...
// File: contracts/Staking/RisqStakingPAXG.sol
...
contract RisqStakingPAXG is RisqStaking, IRisqStakingBEP20 {
    using SafeBEP20 for IBEP20;
    using SafeMath for uint;

    IBEP20 public immutable PAXG;

    constructor(BEP20 _token, BEP20 paxg) public
        RisqStaking(_token, "RISQ PAXG Staking lot", "rPAXG") {
        PAXG = paxg;
    }
    ...
    function sendProfit(uint amount) external override {
        ...
        if (_totalSupply > 0) {
            ...
            PAXG.safeTransferFrom(msg.sender, FALLBACK_RECIPIENT, amount);
        }
    }

    function _transferProfit(uint amount) internal override {
        PAXG.safeTransfer(msg.sender, amount);
    }
}