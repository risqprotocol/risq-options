...
// File: contracts/Staking/RisqStakingGOOGL.sol
...
contract RisqStakingGOOGL is RisqStaking, IRisqStakingBEP20 {
    using SafeBEP20 for IBEP20;
    using SafeMath for uint;

    IBEP20 public immutable GOOGL;

    constructor(BEP20 _token, BEP20 googl) public
        RisqStaking(_token, "RISQ GOOGL Staking lot", "rGOOGL") {
        GOOGL = googl;
    }
    ...
    function sendProfit(uint amount) external override {
        ...
        if (_totalSupply > 0) {
            ...
            GOOGL.safeTransferFrom(msg.sender, address(this), amount);
            emit Profit(amount);
        } else {
            GOOGL.safeTransferFrom(msg.sender, FALLBACK_RECIPIENT, amount);
        }
    }

    function _transferProfit(uint amount) internal override {
        GOOGL.safeTransfer(msg.sender, amount);
    }
}